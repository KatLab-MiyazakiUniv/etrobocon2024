...
void LineTracing::recover()
{
  ...
  // 復帰動作コマンド実行後の角度補正
  int recoveringEndTargetAngle = fabs((diffLeftMileage - diffRightMileage) / TREAD * 180 / M_PI)
                                 / 1.3;  // 緑に入った角度によって回転角を変更
  ...
  ColorLineTracing recoveringEndColorLineTracing(
      recoveringEndTargetColor, recoveringEndTargetSpeed, recoveringEndTargetBrightness,
      recoveringEndPidGainForColorLineTracing, isLeftEdge, recoveringEndIsRecoveryEnabled);
  ...
  DistanceLineTracing recoveringEndDistanceLineTracing(
      recoveringEndTargetDistance, recoveringEndTargetSpeed, recoveringEndTargetBrightness,
      recoveringEndPidGainForDistanceLineTracing, isLeftEdge, recoveringEndIsRecoveryEnabled);
