...
void LineTracing::recover()
{
  ...
  // 復帰動作コマンド実行後の角度補正
  int endTargetAngle = fabs((diffLeftMileage - diffRightMileage) / TREAD * 180 / M_PI)
                       / 1.4;  // 緑に入った角度によって回転角を変更
  ...
  ColorLineTracing endColorLineTracing(endTargetColor, endTargetSpeed, endTargetBrightness,
                                       endPidGain, isLeftEdge, endIsRecoveryEnabled);
  endColorLineTracing.run();

  int endTargetDistance = 300;
  DistanceLineTracing endDistanceLineTracing(endTargetDistance, endTargetSpeed, endTargetBrightness,
                                             endPidGain, isLeftEdge, endIsRecoveryEnabled);
