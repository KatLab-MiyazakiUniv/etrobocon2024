                                            ...
                                            bool& isLeftEdge)
{
  ...
  while(fgets(row, READ_BUF_SIZE, fp) != NULL) {
    ...
    } else if(command == COMMAND::DA) {  // デブリリムーバルエリアの直進補正動作の生成
      DebrisRemovalAction* da = new DebrisRemovalAction(atof(params[1]),  // 直進時の速度
                                                        atoi(params[2]),  // 回頭時のPWM
                                                        atof(params[3])   // 回頭角度のバイアス
      );

      motionList.push_back(da);
    } else if(command == COMMAND::DF) {  // デブリリムーバルエリアの回頭込み直進補正動作の生成
...
COMMAND MotionParser::convertCommand(char* str)
{
  if(strcmp(str, "DL") == 0) {  // 文字列がDLの場合
    ...
    return COMMAND::DA;
  } else if(strcmp(str, "DF") == 0) {  // 文字列がDFの場合
    return COMMAND::DF;
...
bool MotionParser::convertBool(char* command, char* stringParameter)
{
  ...
  if((strcmp(command, "PR") == 0) || (strcmp(command, "AC") == 0) || (strcmp(command, "BC") == 0)
     || (strcmp(command, "DF") == 0)) {    //  コマンドがPR・AC・BC・DFのいずれかの場合
