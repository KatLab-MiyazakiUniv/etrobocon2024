...
class FrameTimingCalculator:
    ...
    def get_target_timing(self):
        ...
        # 黄色い長方形を検出
        yellow_rect = self.detect_yellow_rectangle(
            first_frame, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) / 2)
        )
    ...
    def detect_yellow_rectangle(self, frame, y):
        """配置エリアAにおける背景を検出するため、黄色の矩形を検出する。"""

        upper_frame = frame[0:y, :]

        # HSV色空間に変換
        hsv = cv2.cvtColor(upper_frame, cv2.COLOR_BGR2HSV)
